...
namespace dsf::mobility {
  void TrafficSimulator::connectDataBase(std::string_view const dbPath,
    ...
    m_databasePath = std::string(dbPath);
    m_databaseQueries = std::string(queries);
    m_database = std::make_unique<SQLite::Database>(
        m_databasePath, SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
    m_database->exec(m_databaseQueries);
  ...
  void TrafficSimulator::saveData(std::time_t const savingInterval,
    ...
    m_preparePersistence();
    spdlog::info(
        "Data saving configured: interval={}s, avg_stats={}, street_data={}, travel_data={}, agent_data={}",
        savingInterval,
        saveAverageStats,
        saveStreetData,
        saveTravelData,
  ...
  void TrafficSimulator::setTimeFrame(std::time_t const initTime,
    ...
    if (endTime.has_value()) {
      if (*endTime <= m_initTime) {
        spdlog::warn("End time ({}) is not greater than initial time ({}). End time will be ignored.",
                     *endTime,
                     m_initTime);
      } else {
        m_endTime = *endTime;
  ...
  void TrafficSimulator::setNAgentsPerTimeStep(
    ...
    if (deltaT.has_value()) {
      if (m_endTime == 0) {
        spdlog::warn(
            "Delta time for agent insertion is set to {} seconds, but an end time is already set. The end time will be ignored for agent insertion timing.",
            deltaT.value());
      }
  ...
  void TrafficSimulator::m_initStreetTable() const {
    if (!this->database()) {
      throw std::runtime_error("No database connected. Call connectDataBase() before saving data.");
    }
    this->database()->exec(
  ...
  void TrafficSimulator::m_saveAvgStatsCSV(const std::string& datetime,
    ...
    outFile << datetime << CSV_SEPARATOR << time_step << CSV_SEPARATOR
            << m_dynamics->ghostAgents() << CSV_SEPARATOR << m_dynamics->nAgents() << CSV_SEPARATOR;
    if (averageStats.nValidEdges > 0) {
      outFile << averageStats.meanSpeed << CSV_SEPARATOR << averageStats.stdSpeed << CSV_SEPARATOR
              << averageStats.meanDensity << CSV_SEPARATOR << averageStats.stdDensity
              << CSV_SEPARATOR << averageStats.meanTravelTime << CSV_SEPARATOR;
    } else {
      outFile << CSV_SEPARATOR << CSV_SEPARATOR << averageStats.meanDensity << CSV_SEPARATOR
              << averageStats.stdDensity << CSV_SEPARATOR << CSV_SEPARATOR;
    }
    outFile << averageStats.meanQueueLength << "\n";
    outFile.flush();
    outFile.close();
    spdlog::debug("Saved average stats for time step {} to file {}.", time_step, filename);
  }
  ...
  void TrafficSimulator::m_saveAgentDataSQL(
      const std::int64_t time_step,
      const std::int64_t simulation_id,
      tbb::concurrent_unordered_map<Id, std::vector<std::tuple<Id, std::time_t, std::time_t>>>
  ...
  void TrafficSimulator::m_dumpNetwork() const {
    ...
    if (edgesTableExists && nodesTableExists) {
      spdlog::info("Edges and nodes tables already exist in the database. Skipping network dump.");
      return;
    }
  ...
  void TrafficSimulator::m_flushStepData(StepDataResult&& stepData) {
    ...
    auto const simulationId = static_cast<std::int64_t>(m_dynamics->id());

    if (m_database != nullptr) {
      std::optional<SQLite::Transaction> transaction;
      transaction.emplace(*m_database);
      if (stepData.streetData.has_value()) {
        m_saveStreetDataSQL(datetime, timeStep, simulationId, std::move(*stepData.streetData));
      }
      if (stepData.travelData.has_value()) {
        m_saveTravelDataSQL(datetime, timeStep, simulationId, std::move(*stepData.travelData));
      }
    ...
    if (stepData.streetData.has_value()) {
      m_saveStreetDataCSV(datetime, timeStep, simulationId, std::move(*stepData.streetData));
    }
    if (stepData.travelData.has_value()) {
      m_saveTravelDataCSV(datetime, timeStep, simulationId, std::move(*stepData.travelData));
    }
    if (stepData.agentData.has_value()) {
  ...
  void TrafficSimulator::run(bool const reinsertAgents) {
    if (m_dynamics == nullptr) {
      throw std::runtime_error("Cannot run the simulation without imported road network dynamics.");
    }
    if (m_nAgentsPerTimeStep.empty()) {
      throw std::runtime_error("Cannot run the simulation without an agent insertion schedule.");
    }
    ...
    if (m_endTime == 0) {
      m_endTime = m_initTime +
                  static_cast<std::time_t>(m_agentInsertionDeltaT * m_nAgentsPerTimeStep.size());
    }
    ...
    spdlog::info(
        "Starting simulation run from {} to {} ({} time steps) with agent insertion every {} seconds.",
    ...
    for (auto currentTime = m_initTime; currentTime < m_endTime; ++currentTime) {
      ...
      if (currentStep % m_agentInsertionDeltaT == 0) {
        auto const nAgents = m_nAgentsPerTimeStep.at(static_cast<std::size_t>(currentStep / m_agentInsertionDeltaT));
        if (nAgents > 0) {
            m_dynamics->addAgents(nAgents, m_agentInsertionMethod);
        }
      }

      bool const shouldSave = m_savingInterval.has_value() &&
                              (m_savingInterval.value() == 0 ||
                               currentStep % m_savingInterval.value() == 0);
      auto stepData = m_dynamics->evolve(
          reinsertAgents,
