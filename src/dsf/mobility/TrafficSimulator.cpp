...
namespace dsf::mobility {
  ...
  void TrafficSimulator::m_dumpSimInfoSQL() const {
    ...
    void TrafficSimulator::m_saveStreetDataCSV(
      ...
      if (!fileExists) {
        outFile << "datetime" << CSV_SEPARATOR << "time_step" << CSV_SEPARATOR
                << "street_id" << CSV_SEPARATOR << "coil" << CSV_SEPARATOR
                << "density_vpk" << CSV_SEPARATOR << "avg_speed_kph" << CSV_SEPARATOR
                << "std_speed_kph" << CSV_SEPARATOR << "n_observations" << CSV_SEPARATOR
                << "counts" << CSV_SEPARATOR << "queue_length" << "\n";
      }
      for (auto const& [streetId, record] : streetDataRecords) {
    ...
    void TrafficSimulator::m_saveAvgStatsSQL(const std::string& datetime,
                                             const std::int64_t time_step,
                                             const std::int64_t simulation_id,
                                             const AverageStatsRecord& averageStats)
        const {
      SQLite::Statement insertStmt(
          *this->database(),
    ...
    void TrafficSimulator::m_saveAvgStatsCSV(const std::string& datetime,
      ...
      if (!fileExists) {
        outFile << "datetime" << CSV_SEPARATOR << "time_step" << CSV_SEPARATOR
                ...
                << "mean_density_vpk" << CSV_SEPARATOR << "std_density_vpk"
                << CSV_SEPARATOR << "mean_travel_time_s" << CSV_SEPARATOR
                << "mean_queue_length" << "\n";
      }
      outFile << datetime << CSV_SEPARATOR << time_step << CSV_SEPARATOR
    ...
    void TrafficSimulator::m_saveTravelDataCSV(
      ...
      auto const filename{
          std::format("{}_{}_travel_data.csv", simulation_id, m_safeName)};
      bool fileExists = std::filesystem::exists(filename);
      std::ofstream outFile(filename, std::ios::app);
      if (!outFile.is_open()) {
    ...
    void TrafficSimulator::m_saveAgentDataSQL(
        ...
        tbb::concurrent_unordered_map<
            Id,
            std::vector<std::tuple<Id, std::time_t, std::time_t>>> agentDataRecords)
        const {
    ...
    void TrafficSimulator::m_saveAgentDataCSV(
      ...
      for (auto const& [edge_id, data] : agentDataRecords) {
        for (auto const& [agent_id, ts_in, ts_out] : data) {
          outFile << simulation_id << CSV_SEPARATOR << agent_id << CSV_SEPARATOR
                  << edge_id << CSV_SEPARATOR << ts_in << CSV_SEPARATOR << ts_out << "\n";
        }
      }
      outFile.flush();
      outFile.close();
    ...
    void TrafficSimulator::run(bool const reinsertAgents) {
      ...
      for (std::size_t currentStep{0}; currentStep < totalTimeSteps; ++currentStep) {
        ...
        bool const shouldSave =
            m_savingInterval.has_value() && (m_savingInterval.value() == 0 ||
                                             currentStep % m_savingInterval.value() == 0);
        auto stepData =
            m_dynamics->evolve(reinsertAgents,
